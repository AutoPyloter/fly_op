"""Rerun of the pressure-vessel design task (fly_vessel_design_multiseed.py,
original FAIL delta=-0.031) after finding and fixing a term-balance bug in
benchmarks_vessel.py -- see its random_geometry docstring and EXPERIMENTS.md
2026-09-24. The ORIGINAL cost_weight range left the hoop-stress term
dominating >97% of the objective at typical sampled points (the mirror
image of the bug found and fixed in benchmarks_column.py, which had the
COST term dominating >99.99%). Rebalanced cost_weight=[5e8,4e9] gives a
median stress-term share ~44%; RAY_RADIUS also reduced 0.01 -> 0.001 after
re-verifying finite-difference teacher-signal quality (mean cosine-sim to
a near-exact local gradient reference improved from ~0.92/std=0.30 to
~0.9998/std=0.0018 at this radius).

Writes to SEPARATE result files -- the original (retracted, invalid) FAIL
result is preserved for the record, not overwritten.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_vessel_design_recalibrated_multiseed.jsonl"
    results = {"real": [], "null": []}
    ...
    print("\n=== SONUC (basincli kap tasarimi, YENIDEN KALIBRE EDILMIS, n=8) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_vessel_design_recalibrated_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_vessel_design_recalibrated_summary.json")
