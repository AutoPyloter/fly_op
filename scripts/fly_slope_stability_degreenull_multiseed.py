"""Robustness check on PROTOCOL_V2's strongest PASS result (2026-09-18,
fly_slope_stability_multiseed.py: delta=0.906, single-shot slope-stability
regression, real_connectome vs er_null) -- same architecture, same
subgraph (encode-seed=9000), same training, but against
`degree_preserving_rewire` (PROTOCOL.md's actual primary null) instead of
`er_null`. See EXPERIMENTS.md 2026-09-18 "ONEMLI SAGLAMLIK BULGUSU": the
session's other clean er_null-based finding (CX-modulated, delta=-0.562)
completely vanished (delta=+0.125) against this stricter null -- this
checks whether the SAME degree-distribution-artifact explanation applies
to the strongest PASS result too.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_slope_stability_degreenull_multiseed.jsonl"
    ...
    print("\n=== SONUC (sev stabilitesi, degree-preserving null, n=8 tohum) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_slope_stability_degreenull_summary.json", "w", encoding="utf-8") as f:
        json.dump({"real": real.tolist(), "null": null.tolist(), "wilcoxon_p": float(pvalue), "cliffs_delta": cliffs_delta}, f, indent=2)
    print("wrote results/fly_slope_stability_degreenull_summary.json")
