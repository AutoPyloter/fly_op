"""n=8 pilot (fly_crosstask_zeroshot_transfer_multiseed.py) crossed the
Cliff's-delta magnitude threshold (delta=-0.406, |delta|>0.33) but NOT the
p-value threshold (Mann-Whitney p=0.195). Per this project's standing
protocol (n=8 crossing the MAGNITUDE threshold alone triggers an n=30
extension, regardless of significance at n=8 -- see EXPERIMENTS.md/
PROTOCOL.md), this runs 22 new seeds (8-29) and combines with the
original 0-7 for the official n=30 verdict.

(Correction note, 2026-09-24: the overnight autonomous-battery agent
treated "gate = p<0.05 AND |delta|>0.33" as the sole criterion for
whether to extend, which is NOT this project's actual standing rule --
the rule is extend-on-magnitude-alone. This script fixes that omission
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_crosstask_zeroshot_transfer_seeds8to29.jsonl"
    results = {"real": [], "null": []}
    ...
    orig = [json.loads(l) for l in open("C:/projeler/fly_op/results/fly_crosstask_zeroshot_transfer_multiseed.jsonl", encoding="utf-8")]
    real = np.array([r["real_zeroshot_beam_mse"] for r in orig] + results["real"])
    null = np.array([r["null_zeroshot_beam_mse"] for r in orig] + results["null"])
    ...
    print("\n=== SONUC (capraz-gorev sifir-atis transfer, n=30) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_crosstask_zeroshot_transfer_official30_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_crosstask_zeroshot_transfer_official30_summary.json")
