"""Diagnostic follow-up to fly_vessel_design_multiseed.py's FAIL
(delta=-0.031, tam beraberlik, EXPERIMENTS.md 2026-09-24). Root-cause
check found the original RAY_RADIUS=0.01 gives a teacher-signal cosine
similarity to a near-exact local gradient reference of only mean=0.85
(std=0.51, min=-0.99 -- sometimes literally BACKWARDS), because t's own
parameter range is only ~0.079 -- a 0.01 step is ~12.7% of that range
(vs ~3.5% for beam's b,h and ~1.3-1.9% for slope's 3 params), not a
locally-linear finite-difference probe. RAY_RADIUS=0.001 improves this
to mean=0.96/std=0.25 (still imperfect but much better).

This reruns the IDENTICAL n=8 pilot with RAY_RADIUS=0.001 only, to test
whether a real vs null gap emerges once the teacher signal is less
noisy for BOTH arms. Writes to separate result files -- the original
RAY_RADIUS=0.01 FAIL result is a genuine, already-committed finding and
is not overwritten.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_vessel_design_smallradius_multiseed.jsonl"
    results = {"real": [], "null": []}
    ...
    print("\n=== SONUC (basincli kap tasarimi, kucuk yaricap RAY_RADIUS=0.001, n=8) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_vessel_design_smallradius_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_vessel_design_smallradius_summary.json")
