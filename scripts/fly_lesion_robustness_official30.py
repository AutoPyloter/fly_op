"""n=8 pilot (fly_lesion_robustness_multiseed.py) crossed the gate at ALL
three ablation levels, but in the OPPOSITE direction from the naive
"biological networks are more damage-tolerant" prior: cliffs_delta was
NEGATIVE (~-0.88), meaning the REAL connectome degrades MORE than the
degree-preserving null under random recurrent-edge ablation, not less.
Per this project's standard protocol (n=8 crossing the gate triggers an
n=30 extension regardless of direction), this runs 22 new seeds (8-29)
and combines with the original 0-7 for the official n=30 verdict, at
all three ablation fractions (10/20/30%), official gate at 20%.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_lesion_robustness_seeds8to29.jsonl"
    ...
    # combine with original seeds 0-7
    orig = [json.loads(l) for l in open("C:/projeler/fly_op/results/fly_lesion_robustness_multiseed.jsonl", encoding="utf-8")]
    summary = {}
    for frac in ABLATION_FRACS:
        key = int(frac * 100)
        orig_real = [r[f"real_ratio_{key}"] for r in orig]
        orig_null = [r[f"null_ratio_{key}"] for r in orig]
        real = np.array(orig_real + per_frac_real[frac])
        null = np.array(orig_null + per_frac_null[frac])
        ...
        star = " <== RESMI KAPI (n=30)" if frac == GATE_FRAC else ""
        print(f"\n=== ablation={key}% (n=30) ==={star}")
    ...
    with open("C:/projeler/fly_op/results/fly_lesion_robustness_official30_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)
    print("\nwrote results/fly_lesion_robustness_official30_summary.json")
    print(f"RESMI KAPI (X=20%, n=30): {'PASS' if summary['20']['gate_pass'] else 'FAIL'}")
