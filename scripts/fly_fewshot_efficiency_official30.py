"""n=8 pilot (fly_fewshot_efficiency_multiseed.py) crossed the Cliff's-delta
magnitude threshold (delta=0.469, |delta|>0.33) but NOT the p-value
threshold (Mann-Whitney p=0.130). Per this project's standing protocol
(n=8 crossing the MAGNITUDE threshold alone triggers an n=30 extension,
regardless of significance at n=8), this runs 22 new seeds (8-29) and
combines with the original 0-7 for the official n=30 verdict.

(Correction note, 2026-09-24: see fly_crosstask_zeroshot_transfer_official30.py
for why this extension was missing from the overnight battery's own pass.)
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_fewshot_efficiency_seeds8to29.jsonl"
    results = {"real": [], "null": []}
    ...
    orig = [json.loads(l) for l in open("C:/projeler/fly_op/results/fly_fewshot_efficiency_multiseed.jsonl", encoding="utf-8")]
    ...
    print(f"\n=== SONUC (az-ornekli veri-verimliligi, n=30) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_fewshot_efficiency_official30_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_fewshot_efficiency_official30_summary.json")
