"""Path-integration / dead-reckoning test (2026-09-23/24, overnight
autonomous battery). Genuinely new task category, biologically motivated
by the fly central-complex/ellipsoid-body head-direction system (the
closest real neural analog to "integrate a sequence of movements into a
position estimate" in this animal). Unlike every prior task (constant
injected drive for T steps, single-shot regression), this feeds a
DIFFERENT random 2D step vector at each of T=8 timesteps and trains the
network to output the negative cumulative sum of all steps (the "vector
home") at the final step -- a genuine temporal-integration task, not a
single-shot spatial direction guess.

Uses a custom sequence-unroll forward pass (RateBrain.forward() only
supports a single constant injected vector for all T steps) built from
the same recurrence equations (leaky-rate, Dale's-law weights) and the
same trained brain object/parameters -- architecture and null-model
protocol otherwise identical to every other test in this project.
n=8 seeds, degree_preserving_rewire null, same stats convention.
"""
...
DIM = 2  # (dx, dy) step vector / (x, y) home-vector output
N_ENCODE = 2  # one encode neuron per step-vector component (no finite-difference rays here)
N_READOUT = 30
SUBGRAPH_SIZE = 3000
T_STEPS = 8
EPOCHS = 300
...
def sample_step_sequences(n_seq: int, rng: np.random.Generator):
    """n_seq sequences of T_STEPS random 2D steps, uniform in the unit
    disk (angle uniform, radius uniform in [0,1])."""
    seqs = np.zeros((n_seq, T_STEPS, DIM), dtype=np.float32)
    for i in range(n_seq):
        theta = rng.uniform(0, 2 * np.pi, size=T_STEPS)
        r = rng.uniform(0, 1, size=T_STEPS)
        seqs[i, :, 0] = r * np.cos(theta)
        seqs[i, :, 1] = r * np.sin(theta)
    targets = -seqs.sum(axis=1)  # "way home" vector
    return seqs, targets.astype(np.float32)


def forward_sequence(brain: RateBrain, step_seq_t: torch.Tensor) -> torch.Tensor:
    """step_seq_t: (batch, T, DIM). Returns (batch, DIM) final readout,
    same recurrence as RateBrain.forward() but a different injected
    vector at every timestep instead of a constant one."""
    cfg = brain.config
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_path_integration_multiseed.jsonl"
    results = {"real": [], "null": []}
    with open(log_path, "w", encoding="utf-8") as f:
        for seed in SEEDS:
            train_rng = np.random.default_rng(9500 + seed)
            test_rng = np.random.default_rng(19500 + seed)
            X, Y = sample_step_sequences(N_TRAIN_SEQ, train_rng)
            X_test, Y_test = sample_step_sequences(N_TEST_SEQ, test_rng)
    ...
    print("\n=== SONUC (yol entegrasyonu / dead-reckoning, n=8) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_path_integration_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_path_integration_summary.json")
