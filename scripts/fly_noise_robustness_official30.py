"""n=8 pilot (fly_noise_robustness_multiseed.py) crossed the gate with a
PERFECT separation (Cliff's delta=-1.000, Mann-Whitney p=0.0002) -- same
direction as the lesion-robustness result: the real connectome degrades
MORE than the degree-preserving null under input noise, not less. Per
protocol, extends to n=30 with 22 new seeds (8-29), combined with 0-7.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_noise_robustness_seeds8to29.jsonl"
    ...
    real = np.array([r["real_ratio"] for r in orig] + ratio_real)
    null = np.array([r["null_ratio"] for r in orig] + ratio_null)
    ...
    with open("C:/projeler/fly_op/results/fly_noise_robustness_official30_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_noise_robustness_official30_summary.json")
