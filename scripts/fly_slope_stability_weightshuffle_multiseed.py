"""Third robustness check on PROTOCOL_V2's strongest finding (2026-09-19
follow-up to the degree_preserving_rewire check, which the slope-stability
single-shot result SURVIVED, delta=1.000 -- unlike malecns and
CX-modulated, whose er_null "positive" findings both vanished against
degree_preserving_rewire, see EXPERIMENTS.md "ONEMLI SAGLAMLIK BULGUSU").

This uses `weight_shuffle` instead: EXACT same topology (identical edge
set -- the real wiring diagram, unchanged) with weight magnitudes/signs
permuted among the existing edges. This is a complementary, opposite-
direction test from degree_preserving_rewire (which changes WHO connects
to whom but keeps the degree distribution): weight_shuffle changes HOW
STRONG each real connection is but keeps WHO connects to whom exactly
fixed. If the effect survives this too, topology (not specific weight
magnitudes) is doing the work. If it vanishes here, weight magnitudes
matter, not just wiring.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_slope_stability_weightshuffle_multiseed.jsonl"
    ...
    print("\n=== SONUC (sev stabilitesi, weight-shuffle null, n=8 tohum) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_slope_stability_weightshuffle_summary.json", "w", encoding="utf-8") as f:
        json.dump({"real": real.tolist(), "null": null.tolist(), "wilcoxon_p": float(pvalue), "cliffs_delta": cliffs_delta}, f, indent=2)
    print("wrote results/fly_slope_stability_weightshuffle_summary.json")
