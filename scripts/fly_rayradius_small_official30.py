"""n=8 pilot (fly_rayradius_generalization_multiseed.py) showed the
r=0.1 (10x smaller than training radius) condition crossing the Cliff's-
delta magnitude threshold (delta=0.5, |delta|>0.33) though not p<0.05
(p=0.105). Per standing protocol (magnitude alone triggers an n=30
extension), this runs 22 new seeds (8-29) at r=0.1 only and combines
with the original 0-7 for the official n=30 verdict. r=1.0 (reference)
and r=5.0 already cleared the gate at n=8 and are official as-is.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_rayradius_small_seeds8to29.jsonl"
    real_mses, null_mses = [], []
    ...
    orig = [json.loads(l) for l in open("C:/projeler/fly_op/results/fly_rayradius_generalization_multiseed.jsonl", encoding="utf-8")]
    real = np.array([r["real_mse_r0.1"] for r in orig] + real_mses)
    null = np.array([r["null_mse_r0.1"] for r in orig] + null_mses)
    ...
    with open("C:/projeler/fly_op/results/fly_rayradius_small_official30_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_rayradius_small_official30_summary.json")
