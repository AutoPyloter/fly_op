"""Sixth independent single-shot physical/engineering task (2026-09-24,
user: "bir fiziksel problem daha bul onu da test edelim, sineğin gücünü
gösterelim"). Beam transverse shear design (benchmarks_shear.py) --
mechanically distinct failure mode (shear, not bending/buckling/
deflection/hoop-stress) from all five prior tasks.

Pre-training diagnostic (now standard practice after the column/vessel
recalibration lessons, EXPERIMENTS.md 2026-09-24): invalid rate 8.8%,
shear-term objective share median 25.9% (real tension between the two
terms), finite-difference teacher-signal cosine-sim to a near-exact
local gradient reference = 0.9998 (std=0.0016) at RAY_RADIUS=0.002 --
verified BEFORE spending any GPU time.
...
from flyopt.benchmarks_shear import DIM, shear_cost, random_geometry, sample_valid_point
...
def build_training_set(n_problems: int, n_starts: int, seed: int):
    ...
    for _ in range(n_problems):
        ...
        f = lambda p, geo=geo: shear_cost(p, geo)
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_shear_multiseed.jsonl"
    results = {"real": [], "null": []}
    ...
    print("\n=== SONUC (kiris kesme gerilmesi, n=8) ===")
    ...
    with open("C:/projeler/fly_op/results/fly_shear_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/fly_shear_summary.json")
