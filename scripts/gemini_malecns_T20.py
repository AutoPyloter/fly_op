"""Second cross-individual/sex generalization check (2026-09-21, user:
"gerekirse aynı çalışmayı bir de erkek sinek beyni üzerinden ilerletelim"),
alongside the C. elegans cross-species one. Exact same slope-stability
single-shot regression task, RateBrain architecture, training recipe,
subgraph size and primary null (degree_preserving_rewire) as the official
FlyWire (female) finding (fly_slope_stability_degreenull_multiseed.py,
delta=0.884 n=30) -- only the substrate connectome changes, to the male
Drosophila CNS (see build_malecns_adjacency.py; source data already
downloaded in the sibling fly_demos/malecns project, NOT the same codebase
or task as that project's own DAgger/F1-driving pipeline -- this is our
own official methodology applied to a different connectome).

Question: does the finding hold for a different individual/sex fly's
wiring, not just a different species (C. elegans, which did NOT
replicate -- EXPERIMENTS.md 2026-09-21)?
"""
...
def main():
    ...
    cfg = RateBrainConfig(dim=DIM, n_readout=len(decode_idx), T=20, ray_radius=RAY_RADIUS, decode_scale=0.5, train_gain=True)

    log_path = "C:/projeler/fly_op/results/malecns_slope_stability_degreenull_multiseed.jsonl"
    ...
    print("\n=== SONUC (erkek sinek CNS, sev stabilitesi, degree-preserving null, n=8 tohum) ===")
    ...
    with open("C:/projeler/fly_op/results/malecns_slope_stability_degreenull_summary.json", "w", encoding="utf-8") as f:
    ...
    print("wrote results/malecns_slope_stability_degreenull_summary.json")
