"""Extend beam_design_degreenull_multiseed.py to n=30 (2026-09-21) --
n=8 showed a promising-but-underpowered signal (delta=0.531, Mann-Whitney
p=0.083 -- FAIL but crosses the magnitude threshold, same pattern seen
repeatedly this session), same direction as the slope-stability finding.
Extends per the session's established rule for n=8 threshold-crossing
results.

Runs 22 NEW seeds (8-29), combines with the existing n=8
(beam_design_degreenull_summary.json) for n=30.
"""
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/beam_design_degreenull_seeds8to29.jsonl"
    ...
    with open("C:/projeler/fly_op/results/beam_design_degreenull_seeds8to29_summary.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    with open("C:/projeler/fly_op/results/beam_design_degreenull_summary.json") as f:
        orig = json.load(f)
    ...
    print("\n=== KONSOL KIRIS TASARIMI, DEGREE_PRESERVING NULL, n=30 ===")
    ...
    with open("C:/projeler/fly_op/results/beam_design_degreenull_official30_summary.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("wrote results/beam_design_degreenull_official30_summary.json")
