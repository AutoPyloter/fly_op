"""Extend fly_slope_stability_weightshuffle_multiseed.py's result to the
official n=30 (2026-09-20, following the degree_preserving_rewire result's
official PASS at n=30: Mann-Whitney p~=0.000000, Cliff's delta=0.884).
Current state: n=16 (seeds 0-15), delta=0.305 (just under the |delta|>0.33
magnitude threshold), p=0.211 -- weight_shuffle is not PROTOCOL.md's
designated primary null (degree_preserving_rewire is, and that already
passed officially), but completing this to n=30 too gives full parity
across all three null models at the same sample size.

Runs 14 NEW seeds (16-29), combines with the existing n=16
(fly_slope_stability_weightshuffle_summary_n16.json) for n=30, and
reports both Mann-Whitney U (unpaired, consistent with the official
degree_preserving_rewire test) and Wilcoxon signed-rank (paired,
session convention).
"""
...
LR = 3e-3
RAY_RADIUS = 1.0
SEEDS = list(range(16, 30))  # 14 new seeds; combined with existing 0-15 -> n=30
...
def main():
    ...
    log_path = "C:/projeler/fly_op/results/fly_slope_stability_weightshuffle_seeds16to29.jsonl"
    ...
    with open("C:/projeler/fly_op/results/fly_slope_stability_weightshuffle_seeds16to29_summary.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    with open("C:/projeler/fly_op/results/fly_slope_stability_weightshuffle_summary_n16.json") as f:
        orig = json.load(f)
    combined_real = np.array(orig["real"] + results["real_connectome"])
    combined_null = np.array(orig["null"] + results["weight_shuffle_null"])
    ...
    print("\n=== SEV STABILITESI, WEIGHT_SHUFFLE, n=30 ===")
    ...
    with open("C:/projeler/fly_op/results/fly_slope_stability_weightshuffle_official30_summary.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("wrote results/fly_slope_stability_weightshuffle_official30_summary.json")
