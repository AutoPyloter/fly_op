...
- **Kolon burkulması düzeltmesi (önemli metodolojik ders):** Dördüncü görev (kolon burkulması, Euler elastik kararsızlığı) ilk denemede FAIL verdi (δ=0.172), ama bu sonuç **geçersiz** çıktı — maliyet terimi (cost_weight·b·h) yapısal terimi (P/Pcr) tipik noktalarda %99.99'dan fazla eziyordu (Pcr'nin kesit derinliğine h³ ile aşırı duyarlı olması, [0.03,0.6] kutusunda 5 mertebeye varan bir ölçek uyumsuzluğu yaratıyordu). Bu, beşinci görevin (kiriş sehimi) tanı çıktılarının kolon'a şüpheli derecede benzemesiyle yakalandı. Kutu [0.1,0.3]'e daraltılıp yük referans-kesit oranı olarak yeniden türetildikten sonra: **δ=0.88, p=5×10⁻⁹ (n=30) — ana bulguyla eşdeğer güçte bir PASS.** Fiziksel görev istatistiği artık **3 PASS (şev, kiriş, kolon) / 1 açıklanamamış FAIL (basınçlı kap)**. Ders: "kolay/temiz görünen" bir sonuç bir bulgu değil, kırmızı bayrak olabilir — bu proje bunu tekrar tekrar öğrendi (er_null, PSO bütçesi, ve şimdi bu).
- **Beşinci fiziksel görev (kiriş sehimi/servis-edilebilirlik):** aynı kalibrasyon dersi önceden uygulanarak tasarlandı, test sürüyor.
